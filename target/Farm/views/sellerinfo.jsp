...
<!-- 商家添加的模态框 -->
<div class="modal fade" id="sellerAddModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span>
                </button>
                <h4 class="modal-title" id="myModalLabel">商家添加</h4>
            </div>
            <div class="modal-body">
                ...
                <form class="form-horizontal">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <input type="text" name="sellerCompanyName" class="form-control"
                                   id="sellerCompanyName_add_input"
                                   placeholder="SellerCompanyName">
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="col-sm-2 control-label">地址</label>
                        <div class="col-sm-10">
                            <input type="text" name="sellerAdd" class="form-control" id="SellerAdd_add_input"
                                   placeholder="SellerAdd">
                        </div>
                    </div>
                    <div class="form-group">
...
<!-- 商家修改的模态框 -->
<div class="modal fade" id="sellerUpdateModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span>
                </button>
                <h4 class="modal-title">商家修改</h4>
            </div>
            <div class="modal-body">
                <form class="form-horizontal">
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <input type="text" name="sellerCompanyName" class="form-control"
                                   id="sellerCompanyName_update_input">
                            <span class="help-block"></span>
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="col-sm-2 control-label">地址</label>
                        <div class="col-sm-10">
                            <input type="text" name="sellerAdd" class="form-control" id="sellerAdd_update_input">
                            <span class="help-block"></span>
                        </div>
                    </div>
                    <div class="form-group">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
                <button type="button" class="btn btn-primary" id="seller_update_btn" style="color: #0f0f0f">更新</button>
            </div>
        </div>
    </div>
</div>
        ...
        <div class="col-md-9">
            ...
            <div class="col-md-12" style="background-color:rgba(80,77,77,0.9);height:100%;color:#FFFFFF">
                <div class="row" style="height:50px">
                    <div class="col-md-3" style="margin-top:10px;text-align:center;font-size:20px">商家信息管理</div>
...
<script type="text/javascript">
    ...
    function build_seller_table(result) {
        //清空table表格
        $("#seller_table tbody").empty();
        var seller = result.extend.pageInfo.list;
        $.each(seller, function (index, item) {
            var checkBoxTd = $("<td><input type='checkbox' class='check_item'/></td>");
            var sellerIdTd = $("<td></td>").append(item.id);
            var sellerNameTd = $("<td></td>").append(item.sellerName);
            var sellerPhoneTd = $("<td></td>").append(item.sellerPhone);
            var sellerCompanyNameTd = $("<td></td>").append(item.sellerCompanyName);
            var sellerAddTd = $("<td></td>").append(item.sellerAdd);
            var sellerAlipaynumTd = $("<td></td>").append(item.sellerAlipaynum);
            ...
            $("<tr></tr>").append(checkBoxTd)
                .append(sellerIdTd)
                .append(sellerNameTd)
                .append(sellerPhoneTd)
                .append(sellerCompanyNameTd)
                .append(sellerAddTd)
                .append(sellerAlipaynumTd)
                .append(btnTd)
                .appendTo("#seller_table tbody");
        });
    }
    ...
    $("#seller_add_modal_btn").click(function () {
        $("#sellerAddModal").modal({
            backdrop: "static"
        });
    });

    $("#seller_save_btn").click(function () {
        // 1、模态框中填写的表单数据提交给服务器进行保存
        // 2、发送ajax请求保存商家信息
        $.ajax({
            url: "${APP_PATH}/sellerinfo",
            type: "POST",
    ...
    $(document).on("click", ".delete_btn", function () {
        ...
        if (confirm("确认删除吗？")) {
            //确认，发送ajax请求删除即可
            $.ajax({
                url: "${APP_PATH}/sellerinfo/" + sellerId,
                type: "DELETE",
                success: function (result) {
                    alert(result.msg);
                    //回到本页
    ...
    //点击全部删除，就批量删除
    $("#seller_delete_all_btn").click(function () {
        //
        var del_idstr = "";
        ...
        if (confirm("确认删除吗？")) {
            //发送ajax请求删除
            $.ajax({
                url: "${APP_PATH}/sellerinfo/" + del_idstr,
                type: "DELETE",
                success: function (result) {
                    alert(result.msg);
                    //回到当前页面
    ...
    $(document).on("click", ".edit_btn", function () {
        ...
        function getSeller(id) {
            $.ajax({
                url: "${APP_PATH}/sellerinfo/" + id,
                type: "GET",
                success: function (result) {
                    // console.log(result);
                    var sellerData = result.extend.sellerinfo;
        ...
        $("#seller_update_btn").click(function () {
            //发送ajax请求保存更新的商家数据
            $.ajax({
                url: "${APP_PATH}/sellerinfo/" + $(this).attr("edit-id"),
                type: "PUT",
                data: $("#sellerUpdateModal form").serialize(),
                success: function (result) {
                    // alert(result.msg);
                    //1、关闭对话框
