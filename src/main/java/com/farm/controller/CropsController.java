...
public class CropsController {
    ...
    @ResponseBody
    //pn:pagenumber，即当前页数
    public Msg getCropsWithJson(@RequestParam(value = "pn", defaultValue = "1") Integer pn, Model model,HttpServletRequest request) {
        String cropsname=request.getParameter("cropsname");
        CropsExample cropsExample=new CropsExample();
        if(!StringUtils.isEmpty(cropsname)){
            CropsExample.Criteria criteria=cropsExample.createCriteria();
            criteria.andCropsnameLike(cropsname);
        }
        //pageSize：10，指每页显示的数据数
        PageHelper.startPage(pn, 10);
        List<Crops> crops = cropsServiceImpl.findByCondition(cropsExample);
