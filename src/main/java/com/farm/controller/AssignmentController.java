...
public class AssignmentController {
    ...
    @ResponseBody
    //pn:pagenumber，即当前页数
    public Msg getAssignmentsWithJson(@RequestParam(value = "pn", defaultValue = "1") Integer pn, Model model,HttpServletRequest request) {
        AssignmentExample assignmentExample=new AssignmentExample();
        String assiNickname=request.getParameter("assiNickname");
        if(!StringUtils.isEmpty(assiNickname)){
            AssignmentExample.Criteria criteria=assignmentExample.createCriteria();
            criteria.andAssiNicknameLike(assiNickname);
        }
        //pageSize：10，指每页显示的数据数
        PageHelper.startPage(pn, 10);
        List<Assignment> assignments = assignmentServiceImpl.findByCondition(assignmentExample);
